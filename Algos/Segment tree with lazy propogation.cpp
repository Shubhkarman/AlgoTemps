...
int arr[100],segtree[255],lazytree[255];/// 2 * (closest bigger power of 2) - 1
...
int querytree(int ql, int qh,int l,int h,int pos)
{
    ...
    {
        segtree[pos] = lazytree[pos]%2;
        lazytree[2*pos +1] = lazytree[2*pos +2]= lazytree[pos];
        return querytree(ql,qh,l,h,pos);
    }
}
...
int updatetree(int indx, int n,int val,int l,int h,int pos)
{
    ...
    if(lazytree[pos]==0)
    {
      if(ql<=l && qh>=h)      ///l==h && l==indx for this case
        {
            segtree[pos]= lazytree[pos] = val%2;
            return segtree[pos];
        }
        else if(ql>h || qh<l)
        {
    ...
    else
    {
        segtree[pos] = lazytree[pos]%2;
        lazytree[2*pos +1] = lazytree[2*pos +2]= lazytree[pos];
        return updatetree(ql,n,val,l,h,pos);
    }
}
